...
__global__ void compute_cell_kernel(double *cellStatePtr, double *cellVdend) {
    int n = 0, p,j,k;
    ...
    for(p=j-1;p<=j+1;p++){
        for(q=k-1;q<=k+1;q++){
            if(((p!=j)||(q!=k)) && ((p>=0)&&(q>=0)) && ((p<IO_NETWORK_DIM1)&&(q<IO_NETWORK_DIM2))){
                cellCompParamsPtr[j][k].neighVdend[n++] = cellVdend[i%2][p][q];
                ...
                cellCompParamsPtr[j][k].neighVdend[n++] = cellVdend[i%2][j][k];
            }
        }
    }
...
int main(int argc, char *argv[]){
    ...
    cellStatePtr = malloc(IO_NETWORK_DIM1*IO_NETWORK_DIM2*PARAM_SIZE*sizeof(double));//current and next state
	...
	*/
    ...
    for(i=0;i<IO_NETWORK_SIZE;i++){
        cellStatePtr[i*PARAM_SIZE + STATEADD + b] = -60;//				    cellStateInit[b];
        cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.0337836;// H current	    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.0112788;// High-threshold calciucellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.0049291;// Calcium-dependent potcellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.5;// High-threshold calcium currcellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 3.7152;// Calcium concentration   cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.68; //default arbitrary value bucellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = -60;//				    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 1.0127807;// Sodium (artificial)  cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.3596066;//			    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.7423159;// Low-threshold calciumcellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.0321349;//			    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.2369847;// Potassium (delayed recellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.2369847;//			    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.1;// Potassium (voltage-dependencellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = -60;//				    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.003596066;// Sodium (thalamocortcellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.9;//				    cellStateInit[b];
    	cellStatePtr[i*PARAM_SIZE + STATEADD + b] = 0.2369847;// Potassium (transient)
    }
